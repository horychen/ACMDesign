...
/* PI Control
 * */
double PIDseries(struct PID_Reg *r, double err){
    #define I_STATE r->i_state
    #define I_LIMIT r->i_limit
    double output;
    I_STATE += TS * err * r->Ki * r->Kp; // 积分
...
void CTRL_init(){
    ...
    CTRL.PID_speed.Ki = SPEED_LOOP_PID_INTEGRAL_GAIN;
    CTRL.PID_speed.i_limit = SPEED_LOOP_LIMIT_NEWTON_METER;
    CTRL.PID_speed.i_state = 0.0;
    printf("Speed PID: Kp=%g, Ki=%g, limit=%g Nm\n", CTRL.PID_speed.Kp, CTRL.PID_speed.Ki, CTRL.PID_speed.i_limit);

    CTRL.PID_id.Kp = CURRENT_LOOP_PID_PROPORTIONAL_GAIN; 
    CTRL.PID_id.Ki = CURRENT_LOOP_PID_INTEGRAL_GAIN;
    CTRL.PID_id.i_limit = CURRENT_LOOP_LIMIT_VOLTS; //350.0; // unit: Volt
    CTRL.PID_id.i_state = 0.0;
    printf("Current PID: Kp=%g, Ki=%g, limit=%g V\n", CTRL.PID_id.Kp, CTRL.PID_id.Ki, CTRL.PID_id.i_limit);

    CTRL.PID_iq.Kp = CURRENT_LOOP_PID_PROPORTIONAL_GAIN;
    CTRL.PID_iq.Ki = CURRENT_LOOP_PID_INTEGRAL_GAIN;
...
void control(double speed_cmd, double speed_cmd_dot){
    ...
    if(vc_count++ == SPEED_LOOP_CEILING){
        ...
        CTRL.iq_cmd = - PIDseries(&CTRL.PID_speed, CTRL.omg_ctrl_err);
    ...
    double vd, vq;
    vd = - PIDseries(&CTRL.PID_id, CTRL.id__fb-CTRL.id_cmd);
    vq = - PIDseries(&CTRL.PID_iq, CTRL.iq__fb-CTRL.iq_cmd);
