...
int main(){
    ...
    for(_=0;_<NUMBER_OF_STEPS;++_){
        ...
        if(++dfe_counter == TS_UPSAMPLING_FREQ_EXE_INVERSE){
            ...
            // Voltage command 
            // double vd, vq;
            // CTRL.ual = - PID(&CTRL.PID_id, IS_C(0) - 1.0);

            // Speed control (with current control of course)
            ACM.rpm_cmd = 100;
...
void write_header_to_file(FILE *fw){
    // no space is allowed!
    // fprintf(fw, "x0(id)[A],x1(iq)[A],x2(speed)[rpm],x3(position)[rad],ud[V],uq[V],IS_C(0),IS_C(1)\n");
    fprintf(fw, "Speed\n");
